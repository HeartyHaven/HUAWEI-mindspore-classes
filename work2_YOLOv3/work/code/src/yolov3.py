...
class Iou(nn.Cell):
    ...
    def construct(self, box1, box2):
        box1_xy = box1[:, :, :, :, :, :2]
        box1_wh = box1[:, :, :, :, :, 2:4]
        box1_mins = box1_xy - box1_wh / F.scalar_to_tensor(2.0)
        box1_maxs = box1_xy + box1_wh / F.scalar_to_tensor(2.0)
...
class YoloLossBlock(nn.Cell):
    ...
    def construct(self, grid, prediction, pred_xy, pred_wh, y_true, gt_box):
        ...
        # 计算pred_boxes（预测值，非偏移）和gt_box的IoU值
        best_iou = self.reduce_max(iou, -1) # [batch, grid[0], grid[1], num_anchor]
        # 比较这些IoU值与ignore_threshold值大小，结果为ignore_mask
